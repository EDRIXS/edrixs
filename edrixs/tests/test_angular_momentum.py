...
def test_cf_cubic_d_shape():
    """Cubic CF matrix for d-shell has shape (10, 10)."""
    cf = edrixs.cf_cubic_d(1.5)
    assert cf.shape == (10, 10)


def test_cf_cubic_d_hermitian():
    """Cubic CF matrix is Hermitian."""
    cf = edrixs.cf_cubic_d(1.5)
    assert np.allclose(cf, np.conj(cf.T))


def test_cf_cubic_d_traceless():
    """Cubic CF matrix has zero trace."""
    cf = edrixs.cf_cubic_d(1.5)
    assert np.isclose(np.trace(cf), 0)
...
def test_cf_tetragonal_d_hermitian():
    """Tetragonal CF matrix is Hermitian."""
    cf = edrixs.cf_tetragonal_d(1.5, 0.1, 0.05)
    assert cf.shape == (10, 10)
    assert np.allclose(cf, np.conj(cf.T))


def test_cf_trigonal_t2g_shape_and_hermitian():
    """Trigonal t2g CF matrix has shape (6, 6) and is Hermitian."""
    cf = edrixs.cf_trigonal_t2g(0.3)
    assert cf.shape == (6, 6)
    assert np.allclose(cf, np.conj(cf.T))


def test_cf_trigonal_t2g_zero_delta():
    """Trigonal CF with delta=0 gives zero matrix."""
    cf = edrixs.cf_trigonal_t2g(0.0)
    assert np.allclose(cf, 0)


def test_cf_square_planar_d_hermitian():
    """Square planar CF matrix is Hermitian."""
    cf = edrixs.cf_square_planar_d(1.5, 0.1)
    assert cf.shape == (10, 10)
    assert np.allclose(cf, np.conj(cf.T))


@pytest.mark.parametrize("ll", [1, 2])
def test_get_orb_momentum_matches_components(ll):
    """get_orb_momentum returns array whose components match get_lx/ly/lz."""
    ops = edrixs.get_orb_momentum(ll)
    norbs = 2 * ll + 1
    assert ops.shape == (3, norbs, norbs)
    assert np.allclose(ops[0], edrixs.get_lx(ll))
    assert np.allclose(ops[1], edrixs.get_ly(ll))
    assert np.allclose(ops[2], edrixs.get_lz(ll))


@pytest.mark.parametrize("ll", [1, 2])
def test_get_spin_momentum_matches_components(ll):
    """get_spin_momentum returns array whose components match get_sx/sy/sz."""
    ops = edrixs.get_spin_momentum(ll)
    norbs = 2 * (2 * ll + 1)
    assert ops.shape == (3, norbs, norbs)
    assert np.allclose(ops[0], edrixs.get_sx(ll))
    assert np.allclose(ops[1], edrixs.get_sy(ll))
    assert np.allclose(ops[2], edrixs.get_sz(ll))
