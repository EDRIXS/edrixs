...
def dipole_irrep_map(group='spherical'):
    ...
    -----------
        ...
        D6h, D4h, D3d, D2d, C4v, C3v, D4, C4h :  a2g = (z,)   eg  = (x, y)                uniaxial: parallel / in-plane
        D2h, D2                               :  b1g = (z,)   b2g = (y,)  b3g = (x,)      non-degenerate
        C1                                    :  x   = (x,)   y   = (y,)  z   = (z,)
 
    Returns
    -------
    dict = {irrep: (component indices into sigma1)}, with sigma1 ordered (x, y, z) as fundamental_spectra returns it.
...
def octupole_ops(S_low):
    ...
    Because O^{mu,nu,rho} is already fully symmetric and traceless, each tesseral operator is just the corresponding monomial
    combination of its Cartesian components -- no further projection needed:
    ...
    Returns
    -------
    (3, 3, 3, d0, d0), fully symmetric and Hermitian in the last two indices and has 7 independent components -- the rank-3 tesseral operators '3,0', '3,+-1', '3,+-2', '3,+-3'.  
    ...
    I3 = np.eye(3)
    O = M - (1/5.0)*(np.einsum('ab,cij->abcij', I3, V) + np.einsum('bc,aij->abcij', I3, V) + np.einsum('ca,bij->abcij', I3, V))
    ...
    return {'3,0':  O[z, z, z],
            ...
            '3,+2': O[z, x, x] - O[z, y, y],
            '3,-2': O[x, y, z],
            '3,+3': O[x, x, x] - 3*O[x, y, y],
            '3,-3': 3*O[y, x, x] - O[y, y, y]}
...
def physical_operator_basis(S_low):
    ...
    Description
    -----------
    The raw physical operators -- identity, the three Cartesian pseudospin components, and the five tesseral quadrupoles -- built directly from S_low
    with no ladder recursion. Use this basis whenever you need the amplitudes attached to specific Cartesian operators.
    
    Returns
    -------
    ops, ranks (list of ranks of the ops), labels (list of labels of ops), scale (list of Frobenius norm of the ops) 
    """
    ...
    if d > 2:
        ...
        for irrep, Es in quadrupole_pol_basis('spherical').items():
            ...
            for i, E in enumerate(np.array(Es)[perm_inds]):
                O = np.einsum('mn,mnij->ij', E, Q)
                nrm = np.sqrt(np.real(np.vdot(O, O)))
    ...
    if d > 3:
        ...
        for lab, O in O_reordered_data.items():
            nrm = np.sqrt(np.real(np.vdot(O, O))) 
...
def spin_operator_SU2_basis(S_basis, tol=1e-8):
    ...
    Description
    -----------
    Build Steven operators from exact SU(2) spin operators so the rank d>=2S is automatically zero.
    ...
    for ctr in range(1, max_lim+1):
        ...
        for O, label in zip(ops, labels):
            cand.append((O, ctr, label))

    ops, ranks, labels, scale = [], [], [], []

    for O, k, name in cand:
        nrm = np.sqrt(np.real(np.vdot(O, O)))         # Tr(O^dagger O) = Frobenius norm
        if nrm > tol and k<d:                         # keep only operators that exist for this d0
            
            ops.append(O/nrm);  # normalise the operator to have unit Frobenius norm
            ranks.append(k); 
            labels.append(name);
            scale.append(nrm)
...
def decompose_pseudospin(S_low, tol=1e-8):
    ...
    Split the projected pseudospin into an exact spin times a SPIN-PROJECTION tensor.

        S_low^a  =  sign * sum_b G[a,b] S_exact^b  +  t_a * 1

    with
      G        symmetric positive-definite -- the SPIN-PROJECTION tensor, how much of the ideal spin survives projection.
      Rot      the rotation from the lab axes to the principal axes of G
      S_exact  an exact spin-(d0-1)/2 (SU(2) closes to machine precision), already rotated into the lab frame:  S_exact = Rot . S_ideal
      sign     +1, or -1 when the moment is reversed (normal for a hole)
      t_a      a small trace part, non-zero only because an applied field breaks the exact Kramers/time-reversal structure and zero otherwise.

    Obtained by least-squares fitting S_low on {1, S_ideal} to get Lambda, then polar-decomposing Lambda = G R. Because a rotation preserves SU(2), S_exact = R . S_ideal is still an exact spin, and all the anisotropy sits in the symmetric factor G.
    ...
    d0 = Sg.shape[1]
    S_id = np.array(spin_ops((d0 - 1)/2)[:3]) # only takes Sx, Sy, Sz, not S+/-. These are closed under SU(2) algebra.

    basis = [np.eye(d0, dtype=complex)] + list(S_id) # forms the basis with the identity and the three spin operators

    Gm = np.array([[np.vdot(p, q) for q in basis] for p in basis]) # Gm is diag(d0​, ||S||2, ||S||2, ||S||2)
    coef = np.array([np.linalg.solve(Gm, np.array([np.vdot(p, Sg[a]) for p in basis])) for a in range(3)]) # solves the linear system Gm * coef = <p|S[a]> for each a=0,1,2 (x,y,z) to get the coefficients of S_low in the basis of {1, Sx, Sy, Sz}
    Lam_c = coef[:, 1:];
    ...
    rec = (np.einsum('ab,bij->aij', Lam, S_id) + np.einsum('a,ij->aij', trace_part, np.eye(d0, dtype = complex))) # reconstructs S_low from the coefficients Lam and trace_part
    residual = float(np.abs(Sg - rec).max()/max(np.abs(Sg).max(), 1e-30)) # computes the maximum relative residual between the original S_low and the reconstructed one, normalized by the maximum absolute value of S_low.

    U, sig, Vt = np.linalg.svd(Lam) # performs singular value decomposition on Lam to get U, sig (singular values), and Vt. This is used to decompose Lam into a rotation and a symmetric positive-definite matrix.
    Rot = U @ Vt # computes the rotation matrix Rot from U and Vt
    sign = float(np.sign(np.linalg.det(Rot))) # determines the sign of the determinant of R, which indicates if the rotation is proper or improper (reflection)
    if sign < 0:
        Rot = -Rot # if the determinant is negative, flip the sign of R to make it a proper rotation
    G = np.dot(U,np.dot(np.diag(sig), U.T)) # computes the symmetric positive-definite matrix G from U and the singular values sig
    S_ex_g  = np.tensordot(Rot, S_id, axes=(1,0))                                                                  # gauged basis, rotates the ideal spin operators S_id by the rotation matrix Rot to get S_exact, which is the exact spin in the lab frame.
    S_exact = np.transpose(np.tensordot(Ug, np.tensordot(S_ex_g, np.conj(Ug).T, axes = (2,0)), axes=(1,1)),(1,0,2))# back to the original basis
    return dict(G=G, R=Rot, sign=sign, S_exact=S_exact, Lambda=Lam, g_principal=sig, trace_part=trace_part, residual=residual)
...
def zfs_tensor(H_low, S_pseudo):
    ...
    S  = (d0 - 1)/2
    Sv = list(S_pseudo)
    Q  = np.array([[0.5*(Sv[a] @ Sv[b] + Sv[b] @ Sv[a]) - (a == b)*S*(S + 1)/3*np.eye(d0) for b in range(3)] for a in range(3)]).reshape(9, d0, d0)
...
def rebuild_moment(blocks, ops_all, trace_part=None, bvec=None):
    ...
    Description
    -----------
    m_a from the reported parameters, or H_Z = sum_a b_a m_a when bvec is given (b = mu_B B in eV).  
    It is important use this rather than contracting blocks against a single operator list, since rank 1 and ranks >= 3 carry different normalisations.
...
def Multipole_expansion(hmat_i, hmat_n, emat_i_dim, basis_i, trans_ops, l, gamma = 0.1, ge = 2.0, gap_threshold = 0.05, gap_tol = 1e-9, basis = 'ideal'):
    """
    Parameters
    ----------
    ...
    l             : The orbital angular momentum of the valence shell (s, p, d, f) correspond to l = (0, 1, 2, 3)
    gamma         : The inverse-lifetime of the core hole states. # Note: currently it is a float but it should be entered as an array of floats
    ge            : Needed to generate the net moment, J = L + ge*S
    
    gap_threshold : The gap within which the lowest energy levels remain, user defined - if a gap keeps a few states from what should be a multiplet, an exception is raised. 
    gap_tol       : The difference between the last two energy levels in the multiplet is within a user defined tolerance. 
    basis         : 'ideal' or 'physical' : 'ideal' initiates a Stevens construction of the ladder operators, while 'physical' is a direct way of constructing the operators. 
        
    Description
    -----------
    Generate the projection operator to the lowest num_spin_states eigenstates of the initial Hamiltonian, 
    which is used to project the RIXS cross-section to the low-energy subspace of the initial Hamiltonian.
    ...
    if d0!=d0_raw:
        raise Exception("The gap threshold is chosen such that it slices a multiplet, increase the threshold to " + str(rel[d0] if d0 < len(rel) else np.inf))

    P = evec_i[:, :d0]; # P is the projection operator to the lowest num_spin_states eigenstates of the initial Hamiltonian.
    ...
    M_spin = np.stack([edrixs.build_op(component, None, lb = basis_i, rb = basis_i, backend="dense") for component in M_spin_single_particle]);
    ...
    e_val_n, e_vec_n = np.linalg.eigh(hmat_n); # diagonalize the intermediate Hamiltonian to get eigenvalues and eigenvectors, assumes intermediate Hamiltonian is Hermitian.
    trans_ops_mod = np.tensordot(np.conj(e_vec_n).T, trans_ops, axes=(1, 1)); # transform the transition operators to the eigenbasis of the intermediate Hamiltonian.
    ...
    return dict(
        ...
        g_matrix=g_mat, GG=GG, residual=residual, roundtrip=rt, resid_full=resid_full, resid_ladder = resid_ladder,
        blocks=blocks, ops_all=ops_all, block_labels=labels_G, trace_part=trace_part, D=D, M_low=M_low,
        # RIXS
        B_ops=B_ops, ranks=ranks, labels=labels, scale=scale, trans_ops_mod=trans_ops_mod, e_val_n=e_val_n,
    )
...
def resonant_omegas(eval_n, E0, trans_ops_eig, P, gamma, rel_thresh=1e-2, cluster_tol=None, eps_in=None):
    ...
    trans_ops_eig :    The transition operators from initial to intermediate Hamiltonian but in the initial and intermediate Fock basis
    ...
    Description
    -----------
    Poles of the KH resolvent along omega_in sit at eval_n - E0. Keep the most-spectrally intense (bright) ones (absorption weight > rel_thresh*max) and merge poles
    closer than cluster_tol (default gamma) into intensity-weighted centroids.
    By default we keep the peaks with 1% of the maximum absorption weight, and merge peaks closer than the core-hole lifetime broadening gamma.
    ...
    Aabs = np.tensordot(trans_ops_eig, P, axes=(2, 0));      # <c| D_m |g>, (3, N_n, d0) Alternatively Aabs = np.einsum("mci,ig->mcg", trans_ops_eig, P)
    if eps_in is None:
...
def generate_rixs_coefficients(omega_res, e_val_n, E0, trans_ops_mod, P, gamma, B_ops, verbose=False):
    ...
    E0            :     Ground state energy of the initial Hamiltonian.
    trans_ops_mod :     The transition operators from initial to intermediate Hamiltonian eigenstates  (U_n^\dag T_q).
    P             :     Projection operator from the full basis (valence shell) to lower energy basis.
    gamma         :     The inverse-lifetime of the core hole states. #Note: currently it is a float but it should be entered as an array of floats
    ...
    Returns
    -------
    C             :     Coefficients for each operators in the basis B_ops such that R[w] = \sum C[w]*B_ops, w is resonant omega value.
    ...
    for w, omega_in in enumerate(omega_res):
        ...
        Green = np.diag(1.0 / (e_val_n - z));                                 # evaluate the denominator of the Kramers-Heisenberg formula, which is the Green's function of the intermediate state Hamiltonian.
        R_valence = np.tensordot(np.tensordot(np.conj(np.transpose(trans_ops_mod,(0,2,1))), Green, axes=(2,0)),trans_ops_mod, axes=(2,1));
...
if __name__ == "__main__":
    ...
    # Ni2+ ion in La2NiO4 compound from Fabbris et al., PRL 118, 156402 (2017). Scaling percentage taken from the paper and actual values are obtained from get_atom_data.py script in the edrixs package.
    ...
    out = edrixs.model_1v1c(shell_name=('d', 'p'), shell_level=shell_level_energies, v_soc = valence_spin_orbital_coupling, c_soc = core_spin_orbital_coupling, 
    ...
    hmat_i, hmat_n, trans_ops = edrixs.get_ops(emat_i, umat_i, basis_i, emat_n, umat_n, basis_n, trans_mat, backend="dense")
    
    trans_ops = np.asarray(trans_ops);
    
    emat_i_dim = emat_i.shape[0]
    
    l = 2; # d shell (s:0, p:1, d:2, f:3)
    ...
    output_dict = Multipole_expansion(hmat_i, hmat_n, emat_i_dim, basis_i, trans_ops, l, gamma, G_E, gap_threshold, gap_tol, basis)
    ...
    if resid_ladder[1]>1e-6:
        print("The full residual is "+ str(resid_full) + " after including odd ranked operators upto rank "+str(S_basis.shape[1] if S_basis.shape[1]%2==1 else S_basis.shape[1]-1))
    ...
    omega_res, w_res = resonant_omegas(e_val_n, E0, trans_ops_mod, P, gamma, eps_in = eps_in) # run RIXS only at the bright resonances of isotropic XAS.
    
    C, R_list        = generate_rixs_coefficients(omega_res, e_val_n, E0, trans_ops_mod, P, gamma, B_ops, verbose=True) # generate the RIXS coefficients for each resonance and each polarization pair.
    ...
    ### convert from exact SU(2) operators to the usual definition of the spin operators ###
    ...
    for i in range(len(R_list)):
        R_cart_list[i] = np.tensordot(U_rot_cart.conj().T,np.tensordot(R_list[i], U_rot_cart, axes=(3, 0)), axes = (1,2))
    ...
    w = 0                                      # picking the first resonance for the RIXS cross-section, which is the lowest energy resonance.
    ...
    if num_spin_states > 2:
        ...
        df = pd.DataFrame(F_quad, columns=['{Sx, Sy}', '{Sz, Sy}', '3Sz2 - S(S+1)', '{Sz, Sx}', 'Sx2 - Sy2'], index=['{Sx, Sy}', '{Sz, Sy}', '3Sz2 - S(S+1)', '{Sz, Sx}', 'Sx2 - Sy2'])
        print(df)
    
    if num_spin_states > 3:
        A_octupole = A[ranks == 3]             # (O1, O2, O3, O4, O5, O6, O7) -- the three-magnon amplitudes
        F_oct = np.outer(A_octupole, A_octupole.conj());
        df = pd.DataFrame(F_oct, columns=['y(3x2-y2)', 'xyz', 'y(5z2 - r2)', 'z(5z2 - 3r2)', 'x(5z2 - r2)',  'z(x2-y2)', 'x(x2 - 3y2)'], index=['y(3x2-y2)', 'xyz', 'y(5z2 - r2)', 'z(5z2 - 3r2)', 'x(5z2 - r2)',  'z(x2-y2)', 'x(x2 - 3y2)'])
        print(df)
