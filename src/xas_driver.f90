...
!!
!! Called by xas_fsolver (the f2py-exposed wrapper in pyapi.f90), which is in
!! turn invoked from the Python solvers xas_siam_fort, xas_1v1c_fort and
!! xas_2v1c_fort defined in edrixs/solvers.py.  The Python layer loops over
!! polarisations and rewrites transop_xas.in for each one, so xas_driver is
!! called once per polarisation.
!!
!! Implements the Kramers-Heisenberg XAS cross-section of equation (1) of
!! Wang et al. (CPC 2019),
!!   I_XAS(omega_in, eps_i) = sum_i (1/Z) e^{-E_i / K_B T} sum_n |<n|hat{D}_i|i>|^2
!!                              * Gamma_c/pi / [(omega_in - E_n + E_i)^2 + Gamma_c^2],
!! evaluated via the closure-relation form
!!   I_XAS(omega_in) ~ -(1/pi) Im[ <i| hat{D}_i^dagger (omega_in - H_n + E_i + i*Gamma_c)^{-1} hat{D}_i |i> ]
!! as a continued fraction generated by the Lanczos recurrence on H_n with
!! seed vector |b> = hat{D}_i |i> (paper notation, equations (2), (8), (11)).
!!
!! Workflow (executed once per ground state igs = 1..num_gs):
!!  1. Build the photon-absorption transition operator hat{D}_i as a distributed
!!     CSR matrix mapping the initial-state Hilbert space into the intermediate-
!!     state space (ndim_n rows, ndim_i columns) using build_transop_i.
!!  2. Read the i-th ground-state eigenvector |Gamma_i> from eigvec.igs (written
!!     earlier by ed_driver) and apply hat{D}_i to it: |b> = hat{D}_i |Gamma_i>.
!!     This uses pspmv_csr, the parallel CSR sparse matrix-vector kernel.
!!  3. Build the intermediate-state Hamiltonian H_n with build_ham_n.
!!  4. Run build_krylov_mp on H_n starting from |b> for up to nkryl Lanczos
!!     steps; this yields the alpha/beta continued-fraction coefficients that
!!     parameterise <b|(omega_in - H_n + E_i + i*Gamma_c)^{-1}|b>.
!!  5. Write the Krylov data to xas_poles.igs for post-processing in Python
!!     (read_poles_from_file -> get_spectra_from_poles -> Lorentzian-broadened
!!     spectrum I_XAS(omega_in)).
!!
!! Input files (written by Python before xas_fsolver is called):
!!  - config.in           : control namelist
!!  - hopping_n.in        : intermediate-state hopping (one-body) matrix
!!  - coulomb_n.in        : intermediate-state Coulomb (two-body) matrix
!!  - transop_xas.in      : photon-absorption transition operator
!!  - fock_i.in, fock_n.in: initial and intermediate Fock bases
!!  - eigvec.igs          : ground-state eigenvector(s) from a prior ED run
!!
!! Output files (read by Python):
!!  - xas_poles.igs       : Lanczos coefficients alpha, beta, norm and E_gs
