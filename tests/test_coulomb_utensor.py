...
@pytest.mark.parametrize("shell,slater_integrals", [
    ('d', (4.0, 2.0, 0.5)),
    ('p', (2.0, 1.0)),
    ('t2g', (4.0, 2.0, 0.5)),
])
def test_umat_slater_hermitian_symmetry(shell, slater_integrals):
    """Coulomb tensor satisfies U[i,j,k,l] = conj(U[j,i,l,k])."""
    umat = get_umat_slater(shell, *slater_integrals)
    assert np.allclose(umat, umat.transpose(1, 0, 3, 2).conj())
