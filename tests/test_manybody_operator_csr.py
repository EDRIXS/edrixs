...
from edrixs.solvers import get_H
...
def test_public_get_h_matches_backend_operator_construction():
    """The public ``get_H`` wrapper delegates to the SciPy constructor."""
    basis = fixed_particle_basis(3, 2)
    emat = np.diag([0.2, 0.5, 1.1])
    umat = np.zeros((3, 3, 3, 3), dtype=complex)
    umat[0, 1, 1, 0] = 0.7

    actual = get_H(emat, umat, basis, backend="scipy")
